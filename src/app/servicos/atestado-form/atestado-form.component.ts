...
export class AtestadoFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    params.subscribe(urlParams => {
      ...
      if (this.id) {
        this.service.getAtestadoById(this.id).subscribe(
          response => this.atestado = response, errorResponse => this.atestado = new Atestado())
      }
    })
  }
...
onSubmit(){
  ...
  if (this.id) {
    this.service.putAtestado(this.atestado)
      .subscribe(response => {
        this.success = true;
        this.errors = null;
...
gerarpdf() {
  ...
  documento.setFont("Calibri");
  documento.text("Atestado Médico", 78, 65);
  documento.setFontSize(14);
  documento.text(`       Declaro que o(a) ${this.atestado.prontuario.paciente.paciente}, portador do CPF ${this.atestado.prontuario.paciente.cpf}, compareceu
   ao Instituto Mais Sorrisos, no período das ${this.atestado.chegada} hora às ${this.atestado.saida} horas, recebendo atendimento 
   correspondente ao CID ${this.atestado.cid}, necessitando de ${this.atestado.repouso} dias de repouso.`, 12, 100);
  ...
  documento.text(`DOUTOR - CRO`, 60, 210);
